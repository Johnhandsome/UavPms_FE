...
export const routes: Routes = [
  ...
  { path: 'reset-password', canActivate: [guestGuard], loadComponent: () => import('./features/auth/pages/reset-password/reset-password').then((m) => m.ResetPassword), title: 'Đặt lại mật khẩu | UAV-PMS' },
  {
    ...
    children: [
      ...
      { path: 'missions/:id/inspector', canActivate: [roleGuard(['Inspector', 'Analyst', 'Technician', 'MaintenanceTechnician'])], loadComponent: () => import('./features/missions/pages/mission-inspector/mission-inspector').then((m) => m.MissionInspector), title: 'Tiếp nhận nhiệm vụ | UAV-PMS' },
